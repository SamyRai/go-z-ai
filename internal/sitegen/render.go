...
// Output is plain CSS, scoped under `pre.chroma` so chroma's broad class
// names (.bg, .line, .k, etc.) don't collide with anything else on the page.
func GenerateSyntaxCSS() ([]byte, error) {
	...
	fmt.Fprintln(&out, "/* Auto-generated by cmd/sitegen — Catppuccin Mocha (dark default). */")
	out.Write(dark)
	fmt.Fprintln(&out)
...
// renderScopedTheme runs chroma's WriteCSS for the named style and prefixes
// every selector with `scope` (e.g. `[data-theme="light"] `). Chroma's CSS
// uses selectors like `.chroma .k`, `.bg`, `.chroma .ln` — we prefix each
// comma-separated selector with the scope.
func renderScopedTheme(formatter *chromahtml.Formatter, styleName, scope string) ([]byte, error) {
	...
	// Prefix each comma-separated selector in each line. Chroma emits one
	// rule per line; the part before `{` is the selector list. We split on
	// commas and prefix each piece with `scope`, preserving the leading
	// whitespace (chroma indents continuation selectors) and any "/* … */"
	// comment that lives at the start of the line.
	...
	for _, line := range bytes.Split(buf.Bytes(), []byte("\n")) {
		...
		head := line[:braceIdx]
		body := line[braceIdx:]
		// Chroma emits "/* Comment */ selector" — strip the leading comment
		// (and any whitespace around it) so prefixing produces clean output.
		head = leadingCommentRE.ReplaceAll(head, []byte(""))
		...
		for i, p := range parts {
			...
			scoped.Write(lead)
			scoped.WriteString(scope)
			scoped.Write(trimmed)
		}
		scoped.Write(body)
		scoped.WriteByte('\n')
	}
